...
Config_File::Config_File( string filename, string delimiter,  
	string comment )  
...
{  
	// Construct a Config_File without a file; empty  
}  



bool Config_File::KeyExists( const string& key ) const  
{  
...
/* static */  
void Config_File::Trim( string& inout_s )  
{  
...
{  
	// Save a Config_File to os  
	for( Config_File::mapci p = cf.m_Contents.begin();  
		p != cf.m_Contents.end();  
		++p )  
	{  
...
bool Config_File::FileExist(std::string filename)  
{  
	bool exist= false;  
	std::ifstream in( filename.c_str() );  
	if( in )   
...
void Config_File::ReadFile( string filename, string delimiter,  
	string comment )  
{  
